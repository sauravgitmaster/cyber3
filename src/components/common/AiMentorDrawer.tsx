...
export const AiMentorDrawer: React.FC<AiMentorDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Drawer Panel */}
      <div className="relative w-full max-w-md bg-[#F7F9FC] dark:bg-slate-950 flex flex-col h-full z-10 text-[#243047] dark:text-slate-100 shadow-2xl animate-in slide-in-from-right duration-200 border-l border-slate-200 dark:border-slate-800 transition-colors">
        {/* Header with Byte */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h2 className="text-base font-extrabold text-[#243047] dark:text-slate-100">Ask Byte</h2>
                <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 text-[10px] font-bold border border-blue-200 dark:border-blue-800">
                  AI Cyber Buddy
                </span>
              </div>
        ...
        {insight && (
          <div className="mx-4 mt-4 p-3.5 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-200/70 dark:border-blue-900/60 text-xs space-y-1 shadow-2xs">
            ...
            {onStartRecommendation && insight.recommendationPathId && (
              <button
                ...
                className="mt-1 inline-flex items-center gap-1 text-[11px] font-bold text-[#4F7CFF] dark:text-blue-400 hover:underline cursor-pointer"
              >
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {QUICK_PROMPTS.map((prompt, i) => (
              <button
                ...
                className="whitespace-nowrap px-3 py-1.5 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 shadow-2xs transition-all flex items-center gap-1.5 shrink-0 active:scale-95 cursor-pointer"
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3.5">
                ...
                >
                  ...
                  {isByte && msg.technicalDetails && (
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-[11px] font-bold text-[#4F7CFF] dark:text-blue-400 hover:text-[#3862D9] transition-colors cursor-pointer"
                      >
                      ...
                      {expandedTechId === msg.id && (
                        <div className="mt-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-700 text-[11px] text-slate-700 dark:text-slate-300 font-mono space-y-1">
                          <span className="font-bold uppercase tracking-wider text-[9px] text-slate-600 dark:text-slate-400 block">
                            Deep Dive Breakdown
                          </span>
                  ...
                  <span
                    className={`block text-[10px] text-right ${
                      isByte ? 'text-slate-500 dark:text-slate-400' : 'text-blue-100'
                    }`}
                  >
                    {msg.time}
                  </span>
          ...
          {isTyping && (
            <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 italic pl-8">
        ...
        <div className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
          ...
          >
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-[#F7F9FC] dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:border-[#4F7CFF] focus:bg-white dark:focus:bg-slate-800 text-xs outline-hidden text-[#243047] dark:text-slate-100 placeholder-slate-400 transition-all font-sans"
            />
            <button
              type="submit"
              disabled={!inputText.trim()}
              className="p-2.5 rounded-xl bg-[#4F7CFF] hover:bg-[#3D6CE6] disabled:opacity-40 text-white transition-all shadow-xs cursor-pointer"
